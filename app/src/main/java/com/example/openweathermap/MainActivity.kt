...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.change.setOnClickListener {
            ...
            DisplayDialog(this).show(fm, "Dialog")
//            changeDisplay(isLong)
        }

    }

    fun changeDisplay(long: Boolean) {
        if (long) {
            fm.beginTransaction()
                .replace(R.id.container, LongFragment(ArrayList(weatherInfo), this))
                .commit()
        } else {
            fm.beginTransaction()
                .replace(R.id.container, ShortFragment(ArrayList(weatherInfo), this))
                .commit()
        }
    }
